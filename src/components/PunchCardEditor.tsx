...
import { MusicNote, MusicBoxSettings, CombType } from '../types';
...
// Helper to get pitches list corresponding to comb count (18, 30, 50, 72, or 100)
function getPitchesForComb(combCount: CombType) {
  if (combCount === 18) {
    return [
      { midi: 84, name: 'C6' }, { midi: 83, name: 'B5' }, { midi: 81, name: 'A5' },
      { midi: 79, name: 'G5' }, { midi: 77, name: 'F5' }, { midi: 76, name: 'E5' },
      { midi: 74, name: 'D5' }, { midi: 72, name: 'C5' }, { midi: 71, name: 'B4' },
      { midi: 69, name: 'A4' }, { midi: 67, name: 'G4' }, { midi: 65, name: 'F4' },
      { midi: 64, name: 'E4' }, { midi: 62, name: 'D4' }, { midi: 60, name: 'C4' },
      { midi: 59, name: 'B3' }, { midi: 57, name: 'A3' }, { midi: 55, name: 'G3' },
    ];
  } else if (combCount === 30) {
    const list = [];
    for (let midi = 90; midi >= 55; midi--) {
      list.push({ midi, name: midiToPitchName(midi) });
    }
    return list.slice(0, 30);
  } else if (combCount === 50) {
    // 50弁/50音 (MIDI 97 = C7# から MIDI 48 = C3 まで 50音階)
    const list = [];
    for (let midi = 97; midi >= 48; midi--) {
      list.push({ midi, name: midiToPitchName(midi) });
    }
    return list;
  } else if (combCount === 72) {
    // 72弁/72音 (MIDI 100 から MIDI 29 まで 72音階)
    const list = [];
    for (let midi = 100; midi >= 29; midi--) {
      list.push({ midi, name: midiToPitchName(midi) });
    }
    return list;
  } else {
    // 100弁/100音・無制限 (MIDI 108 = C8 から MIDI 9 = C-1 まで 100音階)
    const list = [];
    for (let midi = 108; midi >= 9; midi--) {
      list.push({ midi, name: midiToPitchName(midi) });
    }
    return list;
  }
...
export default function PunchCardEditor({
  ...
  const [pitchFilter, setPitchFilter] = useState<'all' | 'high' | 'mid' | 'low'>('all');
  ...
  // Get full pitch list according to comb count (18, 30, or 50)
  const fullPitches = useMemo(() => getPitchesForComb(settings.combCount), [settings.combCount]);

  // Filter pitches by octave if 30/50 notes
  const displayedPitches = useMemo(() => {
    if (pitchFilter === 'high') {
      return fullPitches.filter((p) => p.midi >= 72); // C5 and above
    } else if (pitchFilter === 'mid') {
      return fullPitches.filter((p) => p.midi >= 60 && p.midi < 72); // C4 to B4
    } else if (pitchFilter === 'low') {
      return fullPitches.filter((p) => p.midi < 60); // below C4
    }
    return fullPitches;
  }, [fullPitches, pitchFilter]);
  ...
  return (
    <div className="bg-[#2d1b14] border border-[#3d251a] rounded-[32px] p-6 sm:p-8 shadow-xl backdrop-blur-md space-y-6">
      ...
      {/* Octave Range Filter & Pagination Bar */}
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between bg-[#1c0f0a] px-4 py-2.5 rounded-2xl border border-[#3d251a] text-xs gap-3">
        {/* Octave Range Filter */}
        <div className="flex items-center space-x-1">
          <span className="text-[#c19a6b] font-mono text-[11px] mr-1.5 shrink-0">表示音域:</span>
          {(['all', 'high', 'mid', 'low'] as const).map((filter) => {
            const labels = {
              all: `全${fullPitches.length}音`,
              high: '高音域 (C5〜)',
              mid: '中音域 (C4〜B4)',
              low: '低音域 (〜B3)',
            };
            const isSelected = pitchFilter === filter;
            return (
              <button
                key={filter}
                ...
                className={`px-2.5 py-1 rounded-lg text-[11px] transition-colors cursor-pointer ${
                  isSelected
                    ? 'bg-[#c19a6b] text-[#1c0f0a] font-bold shadow-sm'
                    : 'bg-[#2d1b14] text-[#e5d3b3]/70 hover:bg-[#3d251a] hover:text-[#e5d3b3]'
                }`}
              >
                {labels[filter]}
              </button>
        ...
        {/* Time info & Pagination */}
        <div className="flex items-center justify-between sm:justify-end space-x-2">
          <div className="text-[#c19a6b] font-mono text-[11px] mr-2">
            小節 {Math.floor(startBeat / 4) + 1}〜{Math.floor(endBeat / 4)}/全{Math.ceil(maxBeatsCount / 4)}小節（{notes.length}音）
          </div>

          <button
            disabled={page === 0}
            onClick={() => setPage((p) => Math.max(0, p - 1))}
            className="p-1 rounded-lg bg-[#2d1b14] hover:bg-[#3d251a] disabled:opacity-30 border border-[#3d251a] text-[#c19a6b] cursor-pointer"
          >
          ...
          <button
            ...
            className="ml-1 flex items-center space-x-1 px-2.5 py-1 bg-[#3d251a] hover:bg-[#4d3224] text-[#c19a6b] rounded-lg border border-[#4d3224] text-[11px] cursor-pointer"
          >
      ...
      <div className="relative overflow-x-auto rounded-2xl border border-[#3d251a] bg-[#1a0e09] shadow-inner p-3 sm:p-4">
        ...
        <div className="min-w-[720px] select-none">
          ...
          {/* Grid Rows for each pitch with scroll area for 50-notes */}
          <div className="space-y-1 max-h-[480px] overflow-y-auto pr-1">
            {displayedPitches.map(({ midi, name }) => {
              return (
